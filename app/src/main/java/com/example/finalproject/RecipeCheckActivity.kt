...
import com.example.finalproject.databinding.ActivityRecipeCheckBinding
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class RecipeCheckActivity : AppCompatActivity() {

    lateinit var binding: ActivityRecipeCheckBinding
    ...
    var receivedCheckedRecipeSet = ArrayList<recipeData>()

    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityRecipeCheckBinding.inflate(layoutInflater)
    ...
    private fun setRecyclerView()
    {
        ...
        recipe_Adapter = recipeAdapter(receivedCheckedRecipeSet, receivedIngredientSet)
    ...
    private fun navigateToAnotherActivity(item: recipeData, position: Int) {
        val intent = Intent(this@RecipeCheckActivity, ModifyMyRecipeActivity::class.java)
    ...
    {
        receivedCheckedRecipeSet = recipeListDB.recipeData_DAO().findSimilarRecipe(recipeName) as ArrayList<recipeData>
        recipe_Adapter.recipe_items = receivedCheckedRecipeSet
    ...
    var count = 0
    //나만의 레시피들을 가져온다
    fun getCheckedRecipes()
    {
        receivedMyRecipeSet = recipeListDB.recipeData_DAO().getAllRecord() as ArrayList<recipeData>
        ...
        var i = 0
        for(recipes in receivedMyRecipeSet)
        {
            for(recipes_stuff in recipes.recipe_stuff)
            {
                for(ingredient in receivedIngredientSet)
                {
                    if(ingredient.Iname == recipes_stuff && ingredient.Quantity <= recipes.recipe_stuff_count[i])
                    {
                        count++
                        break;
                    }
            ...
            //가지고 있는게 없으면 할 것이 없다 (있는 것만 줘라)
            if(count != 0)
            {
                receivedCheckedRecipeSet.add(recipes)
            }
        }
        //데이터가 유효할 때만
        if(receivedCheckedRecipeSet.isNotEmpty())
        {
            //레시피를 정렬 한다
            ...
            CoroutineScope(Dispatchers.Main).launch{
                ...
                binding.whenRecipeEmpty.visibility = View.GONE
        ...
        else //데이터가 유효하지 않다면
        {
            CoroutineScope(Dispatchers.Main).launch{
                ...
                binding.whenRecipeEmpty.visibility = View.VISIBLE
    ...
    {
        val list_Length = receivedCheckedRecipeSet.count()
        ...
        for(recipe in receivedCheckedRecipeSet)
        {
        ...
        for (pass in 0 until list_Length - 1) {
            ...
            for (j in 1 until list_Length - pass) {
                ...
                if (needed_IngredientTypes[j] < needed_IngredientTypes[j - 1]) {
                    val temp = receivedCheckedRecipeSet[j]
                    receivedCheckedRecipeSet[j] = receivedCheckedRecipeSet[j - 1]
                    receivedCheckedRecipeSet[j - 1] = temp
